...
class DataTransformation():
    ...
    def get_data_transformation_pipeline(self, ) -> Pipeline:
        """
            1. Derived Feature generator
            2. Imputer (Numerical column)
            3. Frequency Imputer (Categorical columns)
            4. StringIndex (Assign index/number to each category)
            5. OneHotEncoder (To perform OHE in pyspark StringIndex is required)
            6. Tokenizer
            7. Hashing
            8. IDF
            9. Vector Assembler (Sum of all the required column)
